...
def main():
    ...
    rows = []
    for path in ('examples.counter_spec:spec', 'examples.peterson_v2:peterson'):
        module, name = path.split(':')
        spec = getattr(importlib.import_module(module), name)
        started = time.monotonic()
        report = verify(spec, directory=args.out/'evidence', timeout=args.timeout)
        evidence = Path(report.evidence)
        rows.append(dict(specification=path, status=report.status, translation=report.translation,
